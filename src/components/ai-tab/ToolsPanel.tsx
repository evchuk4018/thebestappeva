  ...
  return (
    <div className="mt-5 flex flex-1 flex-col gap-3 overflow-y-auto px-3 pb-4">
      <div className="px-1">
        ...
        <p className="mt-1 text-xs leading-relaxed text-zinc-500">Enabled tools are exposed to the model and only load data on demand.</p>
      </div>

      {tools.map((tool) => (
        <div key={tool.id} className="rounded-2xl border border-[#2a2a27] bg-[#171715] p-4">
          ...
          <div className="mt-4 flex flex-col gap-2">
            {tool.functions.map((toolFunction) => (
              <div key={toolFunction.name} className="rounded-xl border border-[#232320] bg-[#11110f] px-3 py-2.5">
